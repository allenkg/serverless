...
```bash
allen-first$ sam build --use-container
```
...
```bash
allen-first$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
allen-first$ sam local start-api
allen-first$ curl http://localhost:3000/
```
...
```bash
allen-first$ sam logs -n HelloWorldFunction --stack-name allen-first --tail
```
...
```bash
allen-first$ pip install -r tests/requirements.txt --user
# unit test
allen-first$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
allen-first$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name allen-first
```
